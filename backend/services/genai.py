...
class YoutubeProcessor:
    ...
    def find_key_concepts(self, documents:list, sample_size: int = 0, verbose = False):
        ...
        if sample_size == 0:
            sample_size = len(documents) // 5 # Consider math.ceil() if the # of documents are smalls
        ...
        for group in tqdm(groups):
            ...
            prompt = PromptTemplate(
                ...
                Respond in the following format as a JSON Object without any labels or additional text. The JSON Object should look exactly like this:
                {{"concept1": "definition1", "concept2": "definition2", ...}}
                """,
                # Ensure "term" and "definition" are assigned as the property name for each concept and definition, respectfully.
                # Respond in the following format as a JSON object without any backticks separating each concept with a comma:
                # {{\\n  "term": "<concept>",\\n  "definition": "<definition>"}},\\n
                # {{\\n  "term": "<concept>",\\n  "definition": "<definition>"}},\\n

                # Follow the instructions to create key concepts:
                # 1. Ensure "term" and "definition" are assigned as the property name for each concept and definition, respectfully.
                # 2. The property names begin the next line for each key concept
                # 3. The property names are indented two spaces for each key concept
                # 4. Only respond the JSON object
                ...
                # Respond in the following format as a JSON object without any backticks separating each concept with a comma. As well as a \\n and two spaces before the first concept in the json:
